...
public class PaginationController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            ArrayList<PostRental> postList = po.getPagingList(index);
            request.setAttribute("listOfPost", postList);
            ArrayList<String> thumbnailList = new ArrayList<>();
            for (int i = 0; i < postList.size(); i++) {
                String url = po.getImageThumbailsByPostID(postList.get(i).getId());
            ...
            for (int i = 0; i < postList.size(); i++) {
                int proID = pros.getPostPromotion(postList.get(i).getPromotion());
                saleList.add(proID);
            }
